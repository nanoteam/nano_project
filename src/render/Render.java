...
public class Render implements Engine {
	...
	private int stateViewPort;
	private float zoom = 1;
	private int omgZerg = 0;
	...
	public void tick() {
		if (stateViewPort == VIEWPORT_ON_PLAYER) {
			...
			glTranslatef((int) (-p.x + Width / 2 / zoom), (int) (-p.y + Height
					/ 2 / zoom), 0);
		} else {
			zoom = 1;
	...
	// TODO add avto detecting optimal working set display, 1600x900x32 or
	// 640x480x16 or ...
	public Render() {
	...
	public void setStateViewPort(int stateViewPort) {
		...
		if (omgZerg % 2 == 0) {
			omgZerg = 0;
			if ((stateViewPort == VIEWPORT_GLOBAL_WORLD)
					|| (stateViewPort == VIEWPORT_ON_PLAYER)) {
