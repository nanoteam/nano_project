...
public class Game {
	...
	public Game(Client client, boolean isTest) {
		if (isTest) {
			...
			menuItems = new MenuItems(client.getRender().getResolutionX(),
					client.getRender().getResolutionY());
	...
	public void start() {
		while (!finished) {
			...
			if (Display.isCloseRequested()) {
				...
				sound.tick();
				render.tick();
				if (state == Game.STATE_GAME_MENU) {
					menuItems.drawGameMenu();
				}
				if (Global.realTime) {
					render.syncFps();
				}
				...
				/*
				 * if (Display.isVisible() || Display.isDirty()) { //Only bother
				 * rendering if the window is visible or dirty render.tick();
				 * //TODO in titurial version this method dosnt call, // but i
				 * think, that call method maut be
	...
	public void sendStatesInput(List<StateKeyboard> eventsKeyboard,
			List<StateMouse> eventsMouses) {
		// keyboard event
		if (eventsKeyboard != null && (!eventsKeyboard.isEmpty())) {
			for (StateKeyboard stateKeyboard : eventsKeyboard) {
				if (stateKeyboard.state == StateKeyboard.DOWN
						|| stateKeyboard.state == StateKeyboard.DOWN_PRESSED) {
					switch (inputToAction.getActionByDevice(
							stateKeyboard.keyCode, InputToAction.KEYBOARD)) {
				...
				if (stateKeyboard.state == StateKeyboard.DOWN_PRESSED) {
					...
					case InputToAction.menu: {
						if (client.isTestMode())
							...
							if (state == Game.STATE_GAME_MENU) {
								state = Game.STATE_GLOBAL_GAME;
							} else {

								state = Game.STATE_GAME_MENU;
							}
						}
						break;
					}
		...
		if (eventsMouses != null && (!eventsMouses.isEmpty())) {
			for (StateMouse stateMouse : eventsMouses) {
				if (!stateMouse.rolickEvent) {
					if (stateMouse.stateButton == StateMouse.DOWN
							|| stateMouse.stateButton == StateMouse.DOWN_PRESSED) {
						switch (inputToAction.getActionByDevice(
								stateMouse.buttonCode, InputToAction.MOUSE)) {
						case InputToAction.firePrimary: {
							level.getPlayer().getControlledObject()
									.doAction(InputToAction.firePrimary);
							break;
	...
	public float getAngleBetweenShipAndCursor() {
		if (render.getStateViewPort() == Render.VIEWPORT_ON_PLAYER) {
			return (float) (Math.PI + Math.atan2(render.getResolutionY() / 2
					- controller.getMousePosition().y, render.getResolutionX()
					/ 2 - controller.getMousePosition().x));
		}

		if (render.getStateViewPort() == Render.VIEWPORT_GLOBAL_WORLD) {
			return (float) (Math.PI + Math.atan2(
					player.getControlledObject().getPosition().y
							- controller.getMousePosition().y,
					player.getControlledObject().getPosition().x
