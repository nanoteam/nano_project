...
final public class Ship extends GameObjectMoving implements ControlledObject {
    ...
    //this code put to construktor engine?
    ...
    public void update() {
        ...
        float angleEngine = leftEngine.getAngle();
        ...
        box[6] = Math.abs(physicObject.getAngle()) / 5f;
    ...
    public void move() {
        ...
        if (turnEnginesLeft) {
            leftEngine.turnOnAngle(0.1f);
            rightEngine.turnOnAngle(0.1f);
        }
        if (turnEnginesRight) {
            leftEngine.turnOnAngle(-0.1f);
            rightEngine.turnOnAngle(-0.1f);
    ...
    public void damage(float... impulses) {
        ...
        cromsonManager.touchWall();

        physicObject.setPosition(new Vector2f(200,200));
        //leftEngine.getPhysicObject().setPosition(new Vector2f(200f - ENGINE_POSITION.x, 200f));
        //rightEngine.getPhysicObject().setPosition(new Vector2f(200f+ENGINE_POSITION.x,200f));

        //position = new Vector2f(200,200);

        physicObject.setSpeed(new Vector2f(0,0));
        physicObject.setAngle(0);
