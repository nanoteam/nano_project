...
public class Ammo extends GamePhysicObject implements IsClonable {
    ...
    public void update() {
        ...
        if (haveForceTraction) {
            level.getEmitterEffects().drawParticlesFromEngine(new Vector2f(position), -angle);
        }
        if (haveAntiMass) {
            level.getEmitterEffects().drawGrowCircles(new Vector2f(position), 4, 50, (Color) Color.GREEN);
