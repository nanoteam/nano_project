...
public class RubberBall extends GameObjectPhysicMoving {
			...
			for (int i = 0; i < 10; i++) {
				level.getNotAddedGameObjects().add(
						new Particle(new Vector2f(position), new Vector2f(
								(random.nextFloat() - 0.5f) * 25f + speed.x, (random
										.nextFloat() - 0.5f) * 25f + speed.y), 3,
								30 + random.nextInt(20) , (Color) Color.GREEN));
			}
		}
	}
	...
	public void draw() {
		...
		//RenderUtil.drawLine();
		RenderUtil.drawQaud(position.x, position.y, width, height, angle, color);
	}
